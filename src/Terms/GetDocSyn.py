...
for topic in Topic.topics_100:
    topic_term_list = FileUtils.openfile(File_Path + str(topic) + prop)
    des = os.path.join(Path.Data_Path + os.sep, "synTerm" + os.sep, status + os.sep, str(topic))
    for term_list in topic_term_list:
        term = term_list.split()
        for t in term:
            try:
                res = model.most_similar(t, topn=30)
